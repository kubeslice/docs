...
3. If the connection issue still persists, check if the KubeSlice Controller endpoint and token in the cluster are correct in the
   ...
   see the [Slice Operator YAML file](/versioned_docs/version-1.0.0/install-kubeslice/yaml/slice-operations/slice-operations-slice-creation.mdx#slice-creation).
   file.
...
Cluster](/versioned_docs/version-1.0.0/install-kubeslice/yaml/yaml-register-worker-clusters.mdx#manually-retrieve-registered-cluster-secrets).
