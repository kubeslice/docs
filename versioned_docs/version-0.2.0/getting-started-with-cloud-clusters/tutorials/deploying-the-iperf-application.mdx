...
2. Apply the manifest file on the project namespace using the following command:
    ```
    kubectl apply -f <slice configuration>.yaml -n <project namespace>
    ```

## Deploying iPerf
In this tutorial, iperf-sleep and iperf-server will be deployed in the two different clusters. 
The cluster used for iperf-sleep is referred to as <sleep cluster\>, and the cluster used for iperf-server is 
referred to as <server cluster\>.
...
Using the template below, create a deployment file name iperf-server.yaml. All fields in the template will 
...
## Uninstalling iPerf
To uninstall iPerf from your KubeSlice configuration, follow the instructions 
