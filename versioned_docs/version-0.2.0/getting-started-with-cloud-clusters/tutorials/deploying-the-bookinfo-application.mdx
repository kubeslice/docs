...
## Introduction
Istio Bookinfo (Bookinfo Application) is a sample application from Istio composed of four separate microservices intended to demonstrate various Istio features. In this topic, we will use Istio Bookinfo to demonstrate inter-slice communications.

## Prerequisites
Before you install Bookinfo, make sure that you have a KubeSlice configuration with two or more clusters registered and meet the prerequisites.

* To install KubeSlice, see Installing KubeSlice.
* Install Istio in all registered worker clusters as described in Installing Istio.
...
## Creating the Slice
To install the BookInfo Application on a single cluster, create a slice without Istio. For more information, see Creating the Slice without External Gateway.
...
To uninstall Istio Bookinfo from your KubeSlice configuration, the instructions offboarding namespaces.
