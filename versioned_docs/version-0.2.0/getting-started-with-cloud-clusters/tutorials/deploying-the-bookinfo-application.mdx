...
## Introduction
Istio Bookinfo ([Bookinfo Application](https://istio.io/latest/docs/examples/bookinfo/)) is a sample application from Istio composed of four separate microservices intended to demonstrate various Istio features. In this topic, we will use Istio Bookinfo to demonstrate inter-slice communications.
...
- You have Istio installed in all registered worker clusters. For more information, see [Installing Istio](/versioned_docs/version-0.2.0/getting-started-with-cloud-clusters/installing-kubeslice/registering-the-worker-cluster.mdx#installing-istio).
- Before creating the slice, create a namespace bookinfo in all participating worker clusters.
Use the following command to create the namespace bookinfo:
  ```
 kubectl create ns bookinfo
 ```
- Inject the istio label to the bookinfo namespace 
 ```
...
## Creating the Slice
To install the BookInfo Application on a single cluster, create a slice without Istio. For more information, see [Creating the Slice without External Gateway](/versioned_docs/version-0.2.0/getting-started-with-cloud-clusters/installing-kubeslice/creating-a-slice.mdx#slice-configuration-without-external-gateways).
...
:::note
These instructions will guide you through deploying the Product Page service to a cluster we will refer to as the <productpage cluster\>, and the remaining services as well as service exports will be deployed to a cluster referred to here as the <services cluster\>.
:::
...
##################################################################################
apiVersion: networking.avesha.io/v1beta1
kind: ServiceExport
metadata:
 name: details
spec:
...
```
Next, use the following command to apply the productpage.yaml file:
```
...
kubectx <services cluster>
```
Use the following command to create the bookinfo namespace that we deploy these services on:
```
kubectl create ns bookinfo
```
Use the following command to label the bookinfo namespace for istio-injection:
```
kubectl label namespace bookinfo istio-injection=enabled
```

Next, using the following commands, apply the details.yaml, ratings.yaml, reviews.yaml, and serviceexports.yaml files.

Run the following command to apply the details.yaml file.
```
...
```
Run the following command to apply the ratings.yaml file:
```
...
```
Run the following command to apply the reviews.yaml file:
```
...
```
Run the following command to verify if the deployed pods are running on the cluster:
```
kubectl get pods -n bookinfo
```
Expected Output
...
```
Run the following command to apply serviceexports.yaml file:
```
kubectl apply -f serviceexports.yaml -n bookinfo
```
Expected Output
```
serviceexport.networking.avesha.io/details created
serviceexport.networking.avesha.io/reviews created
```

## Validating the BookInfo Deployment
### Validating the Services
To verify the Bookinfo installation, switch the contexts to target the <services cluster\>.
```
kubectx <services cluster>
```
...
Switch contexts to target the <productpage cluster\>.
```
...
```
Next, use the following command to check the exposed port for the product page service. In a moment, use this port to visit the Bookinfo webpage.
```
...
```
To view the deployed Bookinfo product page, we need the external IP of an application node and the exposed port we just retrieved. Use the following command to get your node details:
:::note
...
```
Combine the external IP the command returns with the port you retrieved in the last step in the format below, and visit the page in a browser to view your multi-cluster Bookinfo deployment.
```
http://<external ip>:<port>/productpage
```
Example
![alt](/images/Bookinfo-productpage-OS.png)

## Validating the Product page on the Kind Cluster
### Accessing the Product Page from the Local Machine
If the kind clusters are on a local machine, follow these steps:

First, set up port-forwarding from a local machine using the following command:
```
...
If the kind clusters are on Cloud EC2 Machine, follow these steps:

First, connect to your EC2 machine using SSH from your local machine using the following command:
```
...
![alt](/images/Bookinfo-productpage-OS.png)
:::success
You have successfully deployed Istio Bookinfo on a KubeSlice configuration containing at least two clusters.
:::

## Uninstalling Istio BookInfo
To uninstall Istio Bookinfo from your KubeSlice configuration, the instructions [offboarding namespaces](/versioned_docs/version-0.2.0/getting-started-with-cloud-clusters/uninstalling-kubeslice/offboarding-namespaces.mdx).
