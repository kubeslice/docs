...
kubectx <cluster name>
```
2. Apply the iperf-sleep.yaml deployment using the following command:
```
kubectl apply -f iperf-sleep.yaml -n iperf
```

## Creating the iPerf Server YAML File
Using the template below, create a deployment file name iperf-server.yaml. All fields in the template will remain 
