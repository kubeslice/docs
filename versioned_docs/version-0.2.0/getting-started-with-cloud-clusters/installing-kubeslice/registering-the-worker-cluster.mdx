# Registering the Worker Cluster

## Introduction 
Before creating a slice across your Kubernetes clusters, you must
register your worker clusters with the KubeSlice Controller.


## Installing Istio

You must install Istio in the worker clusters for configuring the external gateways.

> You can skip these steps if you have already installed the recommended Istio version in the cluster.

To begin, switch the context to the cluster that will be registered with the KubeSlice Controller.

``` 
kubectx <cluster name>
```

Create the namespace `istio-system` using the following command:

``` 
kubectl create ns istio-system
```

Use the following commands to install Istio:

``` 
helm install istio-base kubeslice/istio-base -n istio-system
helm install istiod kubeslice/istio-discovery -n istio-system
```

Repeat these steps to install Istio in other
worker clusters.

## Validating the installation of Istio 

Validate the installation by checking the pods belonging to the namespace `istio-system`.


``` 
kubectl get pods -n istio-system
```

Example Output

``` 
NAME                          READY   STATUS    RESTARTS   AGE
istiod-66f576dd98-jtshj       1/1     Running   0          100s
```



## Creating the Cluster Registration YAML File

The following information is required to create your cluster registration **.yaml** file.

>Currently, only Azure Kubernetes Service (AKS) and Google Kubernetes Engine(GKE) are supported.

>Register the Worker clusters with the KubeSlice Controller by applying the manifest file on the Project namespace.

>The cluster registration .yaml file can have multiple clusters listed to register with the controller.

Create the cluster registration YAML file using the
following template.

To understand more about the configuration parameters, see [Cluster
Registration Configuration
Parameters]
``` 
apiVersion: controller.kubeslice.io/v1alpha1
kind: Cluster
metadata:
  name: <cluster-name-1>
  namespace: kubeslice-<projectname>
spec:
  networkInterface: <network_interface>
  clusterProperty:
    geoLocation:
      cloudProvider: "<cloud_provider>"
      cloudRegion: "<cloud_region>"
---
apiVersion: controller.kubeslice.io/v1alpha1
kind: Cluster
metadata:
  name: <cluster-name-2>
  namespace: <kubeslice-projectname>
spec:
  networkInterface: <network_interface>
  clusterProperty:   
    geoLocation:
      cloudProvider: "<cloud_provider>"
      cloudRegion: "<cloud_region>"
```

Example YAML file


``` 
apiVersion: controller.kubeslice.io/v1alpha1
kind: Cluster
metadata:
  name: worker-cluster-1
  namespace: avesha
spec:
  networkInterface: eth0
  clusterProperty:   
    geoLocation:
      cloudProvider: "AZURE"
      cloudRegion: "eastus"
---
apiVersion: controller.kubeslice.io/v1alpha1
kind: Cluster
metadata:
  name: worker-cluster-2
  namespace: avesha
spec:
  networkInterface: eth0
  clusterProperty:   
    geoLocation:
      cloudProvider: "AZURE"
      cloudRegion: "westus2"
```


Example Manifest File Only with Mandatory Parameters


``` 
apiVersion: controller.kubeslice.io/v1alpha1
kind: Cluster
metadata:
  name: worker-1
  namespace: kubeslice-avesha
spec:
  networkInterface: eth0
---
apiVersion: controller.kubeslice.io/v1alpha1
kind: Cluster
metadata:
  name: worker-2
  namespace: kubeslice-avesha
spec:
  networkInterface: eth0
```

## Applying the Cluster Registration YAML File 

The following information is required.

| **Values**      |           **Description** |
|----|----|
|  \<cluster name\>  |          The name of the cluster.|
|  \<cluster registration\> |   The name of the registration file.|
| \<project namespace\>  |    The namespace of your project.|
  
You must apply the cluster registration YAML file on your Project namespace. To begin, switch the context to the cluster where the KubeSlice Controller is installed.

``` 
kubectx <cluster name>
```
Run the following command to apply the file.

``` 
kubectl apply -f <cluster registeration>.yaml -n <project namespace>
```

>You have successfully registered the Worker clusters with the KubeSlice
Controller.


## Installing the Slice Operator 

Install the Slice Operator on the worker cluster that you just registered.

Before installing the Slice Operator in your registered cluster, you must get the secrets and the endpoints from the KubeSlice Controller.


### Getting the Secrets of the Registered Cluster 

After registering the worker cluster with the KubeSlice Controller, you
will get a secret listed under the project namespace. The secret
contains access information for the Slice Operator on the worker cluster
to communicate with the KubeSlice Controller.

1.  Switch the context to the controller cluster.

    ``` 
    kubectx <cluster name>
    ```
2.  Get the list of secrets belonging to the project namespace using the following command:

    ``` 
    kubectl get secrets -n kubeslice-<projectname>
    ```
    Example
    ``` 
    kubectl get secrets -n avesha
    ```
    
    Example Output

    ``` 
    NAME                                           TYPE                                  DATA   AGE
    default-token-q2gp9                            kubernetes.io/service-account-token   3      43h
    kubeslice-rbac-ro-abc-token-kp9tq              kubernetes.io/service-account-token   3      43h
    kubeslice-rbac-ro-xyz-token-vcph6        kubernetes.io/service-account-token   3      43h
    kubeslice-rbac-rw-abc-token-vkhfb              kubernetes.io/service-account-token   3      43h
    kubeslice-rbac-rw-xyz-token-rwqr9            kubernetes.io/service-account-token   3      43h
    kubeslice-rbac-worker-aks-worker-1-token-hml58   kubernetes.io/service-account-token   5      43h
    kubeslice-rbac-worker-aks-worker-2-token-lwzj2   kubernetes.io/service-account-token   5      43h
    ```
    
    The name of the secret is in this
    format:` kubeslice-rbac-<registered cluster secret name>-token.`\
    For example, `kubeslice-rbac-worker-aks-worker-1-token-hml58` - this
    secret is meant for a worker cluster that was registered using the
    name aks-worker-1.

3.  Retrieve the details of the secret using the following command:

    ``` 
    kubectl get secrets <worker-cluster-secret-name> -o yaml -n kubeslice-<projectname>
    ```
    
    Example Output

    
    ``` 
    $ kubectl get secrets kubeslice-rbac-worker-aks-worker-1-token-hml58 -o yaml -n  kubeslice-cisco
    apiVersion: v1
    data:
      ca.crt: 
      clusterName: YWtzLXNwb2tlLTE=
      controllerEndpoint: aHR0cHM6Ly8xMjcuMC4wLjE6MzY1MTU=
      namespace: a3ViZXNsaWNlLWNpc2Nv
      token: 
    kind: Secret
    metadata:
      annotations:
        kubernetes.io/service-account.name: kubeslice-rbac-worker-aks-worker-1
        kubernetes.io/service-account.uid: 0c0fb6a2-6e7f-4445-964e-403ff89873eb
      creationTimestamp: "2022-03-15T08:48:04Z"
      managedFields:
      - apiVersion: v1
        fieldsType: FieldsV1
        fieldsV1:
          f:data:
            .: {}
            f:ca.crt: {}
            f:namespace: {}
            f:token: {}
          f:metadata:
            f:annotations:
              .: {}
              f:kubernetes.io/service-account.name: {}
              f:kubernetes.io/service-account.uid: {}
          f:type: {}
        manager: kube-controller-manager
        operation: Update
        time: "2022-03-15T08:48:04Z"
      - apiVersion: v1
        fieldsType: FieldsV1
        fieldsV1:
          f:data:
            f:clusterName: {}
            f:controllerEndpoint: {}
        manager: manager
        operation: Update
        time: "2022-03-15T08:48:34Z"
      name: kubeslice-rbac-worker-aks-worker-1-token-hml58
      namespace: kubeslice-cisco
      resourceVersion: "21121"
      uid: 611af586-b11d-45d4-a6e0-cee3167e837c
    type: kubernetes.io/service-account-token
    ```
    
> Alternatively, you can use the script to create your values .yaml file.
see [Script to get the secrets of the Worker
cluster].

## Creating the Values .yaml file

Use base64 encoded values of the `namespace`, `endpoint`, `ca.crt`, and
the `token `from the above secrets to create your values`.yaml` file.

Create the values `.yaml` file using the following template.

To understand more about the configuration parameters, see [Slice
Operator Configuration
Parameters](https://avesha.atlassian.net/wiki/spaces/TD/pages/2303492097/June+14+Release+Configuration+Parameters#slice-operator-configuration-parameters "https://avesha.atlassian.net/wiki/spaces/TD/pages/2303492097/June+14+Release+Configuration+Parameters#slice-operator-configuration-parameters")

```
Base64 encoded secret values for the namespace, endpoint, ca.crt and token from the controller cluster
controllerSecret:
  namespace: <encoded_namespace> 
  endpoint: <encoded_endpoint>
  ca.crt: <encoded_ca.crt>
  token: <encoded_token>

cluster:
  name: <cluster-name>
```


Example

``` 
controllerSecret:
  namespace: a3ViZXNsaWNlLWF2ZXNoYQ==
  endpoint: aHR0cHM6Ly8xNzIuMTguMC4yOjY0NDM=
  ca.crt: 
  token: 

cluster:
  name: cluster-worker-1
```

## Applying the Values File

The following information is required to apply the manifest file.

| **Parameter**  |   **Description** |
|----|----|
|  \<cluster name\>  | The name of the cluster.|
|  \<values\>        | The file name with the values.|

You must install the Slice Operator in the registered cluster. To begin, switch the context to the registered cluster.

``` 
kubectx <cluster name>
```

Run the following command to apply the values file on the namespace `kubeslice-system`:

``` 
helm install kubeslice-worker kubeslice/kubeslice-worker -f <full path of secrets file>.yaml -n kubeslice-system  --create-namespace
```

> You must apply the values **.yaml** file on each worker cluster to install the Slice operator.

>You have successfully installed the Slice Operator in the Worker cluster. Repeat the above steps to install the Slice Operator in all the
registered worker clusters.


## Validating the Slice Operator Installation 

To validate the Slice Operator installation in the registered cluster,check the pods belonging to the namespace `kubeslice-system` by running
the following command:

Example

``` 
kubectl get pods -n kubeslice-system
```
Example Output

``` {.syntaxhighlighter-pre syntaxhighlighter-params="brush: java; gutter: false; theme: Confluence" theme="Confluence"}
NAME                                    READY   STATUS    RESTARTS   AGE
jaeger-65c6b7f5dd-dmhnl                 1/1     Running   0          41s
kubeslice-netop-2h687                   1/1     Running   0          41s
kubeslice-operator-58fbd594c4-gkb6s     2/2     Running   0          41s
mesh-dns-bf5d97cf5-rzmqb                1/1     Running   0          41s
nsm-admission-webhook-bb4994c7d-m7ssn   1/1     Running   0          41s
nsm-kernel-forwarder-kffkt              1/1     Running   0          40s
nsm-kernel-forwarder-ps9l9              1/1     Running   0          39s
nsmgr-2zmgw                             3/3     Running   0          40s
nsmgr-gt7qz                             3/3     Running   0          41s
prefix-service-76bd89c44f-pgdn7         1/1     Running   0          41s
```

>You have successfully installed the Slice Operator in the worker
cluster. Repeat the above steps to install the Slice Operator in all the
worker clusters.