...
## Introduction
This topic describes the steps to on-board namespaces to a slice. A namespace in Kubernetes 
...
Namespaces that are created to run application deployments can be on-boarded on a slice to 
...
## on-boarding Namespaces
To on-board namespaces, you must add them as part of applicationNamespaces in the [slice configuration](/versioned_docs/version-0.2.0/getting-started-with-cloud-clusters/installing-kubeslice/creating-a-slice.mdx#creating-the-slice-yaml-file) YAML file.
...
:::info
Ensure that the namespace that you want to on-board exists on the worker cluster.
:::
...
is on-boarded on all the worker clusters of that slice.
:::
...
on-boarded automatically on to the slice.

## off-boarding Namespaces
You can off-board namespaces from the slice and prevent the applications on the slice 
from accessing them. To know more, see [off-boarding namespaces](/versioned_docs/version-0.2.0/getting-started-with-cloud-clusters/uninstalling-kubeslice/off-boarding-namespaces.mdx).
