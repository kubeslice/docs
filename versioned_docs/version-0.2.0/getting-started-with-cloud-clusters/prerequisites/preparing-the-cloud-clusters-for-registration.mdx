...
If you have multiple node pools on your cluster, then you can add a label to each node pool. Labels are useful 
in managing scheduling rules for nodes.
...
2.  Run the following command to get the <node name\> and <cluster name\> from the list of nodes belonging to 
