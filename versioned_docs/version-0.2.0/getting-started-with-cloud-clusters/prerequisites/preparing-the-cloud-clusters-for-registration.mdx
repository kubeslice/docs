...
:::info
If a worker cluster contains only one node pool, follow the instructions in [Labeling Individual Nodes](#labeling-individual-nodes).
:::
...
### Elastic Kubernetes Service 
The following information is required to label the EKS cluster nodepools. 
