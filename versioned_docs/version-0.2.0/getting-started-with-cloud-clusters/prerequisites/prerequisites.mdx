...
This topic describes the prerequisites for installing KubeSlice. Ensure
that you have the following environment set up that is required to
install KubeSlice. In this environment, you must install the KubeSlice
Controller and register clusters with it.
...
:::note
You can install the KubeSlice Controller on a cluster and also register
the same cluster with the KubeSlice Controller.
:::
...
| **Supported Kubernetes Services** | Azure Kubernetes Service (AKS) and Google Kubernetes Engine(GKE) |
...
:::caution
Only the cluster administrator can install the KubeSlice Controller and
the Slice Operator(Worker Operator).
:::
