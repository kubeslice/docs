...
Namespaces that are created to run application deployments can be onboarded on a slice to form a micro network segment. Once a namespace is bound to a slice, all the pods that get scheduled in the namespace would get connected to the slice. A brownfield application namespace can be onboarded. The configuration is part of the slice YAML file. This feature eliminates onboarding each application, editing the application’s deployment specification, and adding the slice annotation, which is no longer supported now.
...
> :warning: If you upgrade the worker operator to this 0.2.0 version, then the existing onboarded applications do not work as expected.
...
1. Edit the slice configuration YAML file to add namespaces as part of applicationNamespaces . You can add namespaces in the following ways in the slice configuration YAML file:
...
## Namespace Isolation with Network Policy
...
The supported Kubernetes versions for cloud clusters are 1.20, 1.21, and 1.22. The supported Kubernetes versions for KIND clusters are 1.21 and 1.22.
