...
[create an external QoS profile](#creating-a-qos-profile). Add the name of the external 
...
#### Creating the QoS Profile YAML File
Use the following template to create a `sliceqosconfig` file. 

:::info
To understand more about the configuration parameters, see 
[Slice QoS Profile Parameters](/versioned_docs/version-0.3.0/getting-started-with-cloud-clusters/installing-kubeslice/configuration-parameters.mdx#slice-qos-profile-parameters).
:::
...
spec:
  ...
  dscpClass: AF11
```
#### Applying the QOS Profile YAML File
...
### Validating the QoS Profile 
To validate the QoS profile that you created, use the following command: 
...
slice configuration YAML file as illustrated in the following example. 
