...
:::note
If the KubeSlice Controller is already installed, the '-s controller' option skips installing it. The '-s kind' option skips creating new kind 
clusters; the '-s enterprise' option skips using enterprise installation; and the '-s demo' option skips creating a demo set up.
:::
...
- Create a slice configuration YAML file (choose the namespaces, clusters, etc to be part of the slice).
- Use the `kubeslice-cli create sliceConfig` command to apply the slice configuration YAML file.
