...
To install the Kubeslice controller on the controller cluster and install a worker-operator on the worker cluster, use of one the following methods:
- Setup the `minimal-demo` or `full-demo` installation. Use the kubeslice-cli install command, which creates three clusters: ks-ctrl, ks-w-1, and ks-w-2.
- Install the controller and one or two worker clusters in existing configuration using a custom topology YAML file. use the kubeslice-cli install command the **--config** option.
...
```
The following is the example topology file to register a new worker cluster.
```
configuration:
  cluster_configuration:
...
```
To register a new worker cluster with the KubeSlice Controller in existing mutli-cluster, use the following command:
:::note
