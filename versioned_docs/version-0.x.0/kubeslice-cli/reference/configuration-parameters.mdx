...
This topic describes the parameters used in creating the topology YAML file.
...
|----|----|----|----|
|profile|String|It is the KubeSlice profile to create demo clusters.The support values are **minimal-demo** and **full-demo**|Mandatory|
|kube_config_path|String|It is the absolute path to the kubeconfig file to use topology setup; topology only| Optional|
...
|kube_config_path|String|It is the absolute path to the kubeconfig file to install controller cluster. This takes precedence over the kubeconfig path defined under cluster configuration.|Mandatory|
|control_plane_address|AlphaNumeric|It is the address of the control plane kube-apiserver. The kubeslice-cli determines the address from kubeconfig. Override this flag if the address in kubeconfig is not reachable by other clusters in topology.|Mandatory|
|node_ip|Numeric|It is the IP address of one of the node in the cluster. The kubeslice-cli determines the address from this command:`kubectl get nodes`. Override this flag to an address which is discoverable by other clusters in the topology.| Mandatory|
...
|context_name|String|It is the name of the context to use from kubeconfig file; for topology only|Optional|
|kube_config_path|String|It is the absolute path to the kubeconfig file to install a worker cluster. This takes precedence over the kubeconfig path defined under cluster configuration.|Mandatory|
|control_plane_address|AlphaNumeric|It is the address of the control plane `kube-apiserver`. The kubeslice-cli determines the address from kubeconfig. Override this flag if the address in kubeconfig is not reachable by other clusters in topology.|Mandatory|
|node_ip|Numeric|It is the IP address of one of the node in the cluster. The kubeslice-cli determines the address from this command:`kubectl get nodes`. Override this flag to an address which is discoverable by other clusters in the topology.| Mandatory|
