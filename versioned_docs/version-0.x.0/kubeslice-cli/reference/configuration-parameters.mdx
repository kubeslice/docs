...
:::note
In the topology configuration, the `kube_config_path` parameter requires the absolute path to the kubeconfig 
file. You must set the kube_config_path parameter at the top-level, under `cluster_configuration` or set the `kube_config_path` 
under the controller and the workers configuration.
:::
