# KubeSlice CLI Demo
This topic describes the steps to install the KubeSlice Controller and its components using kind for non-production use. To 
...
8. Creates the `iperf` namespace for application deployment.
...
After successfully installing the KubeSlice Controller and KubeSlice Worker, you can create a slice to onboard your application.

### Creating the Slice Configuration YAML File
:::info
To understand more about the configuration parameters, see [Slice Configuration Parameters](/versioned_docs/version-0.2.0/getting-started-with-cloud-clusters/installing-kubeslice/configuration-parameters.mdx#slice-configuration-parameters).
:::
...
To validate the service imports on the workers clusters, use the below commands.
