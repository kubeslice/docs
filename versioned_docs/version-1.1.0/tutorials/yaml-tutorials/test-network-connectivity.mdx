...
more, see [deploying the iPerf application on an intra-cluster slice](/versioned_docs/version-1.1.0/tutorials/yaml-tutorials/deploying-the-iperf-application-on-an-intra-cluster-slice.mdx).
:::
...
2. Onboard the existing `iperf` namespace to the slice. To know more, see
   [onboarding namespaces](/versioned_docs/version-1.1.0/install-kubeslice/yaml/slice-operations/slice-operations-slice-creation.mdx#manage-namespaces).
