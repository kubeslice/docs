...
* We have identified a scenario where, during a Helm upgrade on kubeslice-worker, the NSM (Network Service Mesh) admission 
  ...
  with the old NSM webhook pod lingering from the previous release. Our team is actively working on a resolution for 
  this problem. Until the fix is implemented, users are advised to manually delete the old NSM webhook pod if encountered
