...
In this demonstration, we will be using three Kubernetes clusters called k8s-cluster-1, k8s-cluster-2, and k8s-cluster-3. In the
k8s-cluster-1 we will install the KubeSlice Controller,the KubeSlice Worker, the MongoDB Master, and the MongoDB member.
...
2. Verify the `merged.config` file:
...
## Install the Kubeslice using the Kubeslice-cli

To install KubeSlice Controller and Worker:

1. Install the latest **kubeslice-cli** tool on your local depending on your OS. 
   For more information, see [Installing kubeslice-cli](https://kubeslice.io/documentation/open-source/1.3.0/get-started/prerequisites/prerequisites-kubeslice-cli-install).
   
   The following is the command to download the tool:
   ```yaml
...
2. Prepare the Kubeslice cluster topology file that you will use by setup Kubeslice. Refer to the 
  [sample topology file](https://github.com/kubeslice/kubeslice-cli/blob/master/samples/template.yaml) and update the values 
  corresponding to your cluster

  Sample topology file can be downloaded from here https://github.com/kubeslice/kubeslice-cli/blob/master/samples/template.yaml

Replace the values in the above topology file as per your cluster details


The following the sample configuration file:
...
2. Use the following command to apply the topology file to install Kubeslice:

   ```
   kubeslice-cli --config kubeslice-topology.yaml install
   ```


## Create a Slice

1. Create a slice and onboard mongodb namespace on it, set the context to your kubeslice controller cluster. 
...
2. Apply the slice configuration yaml file in the project namespace on the controller cluster
   
   Example

   ```
   kubectl apply -f mongodb-slice.yaml -n kubeslice-mongodb-project
   ```

   Example Output

   ```
   sliceconfig.controller.kubeslice.io/mongodb-slice created
   ```

## Deploy the MongoDB Enterprise Kubernetes Operator

Reference Documentation : Deploying the MongoDB Enterprise Kubernetes Operator on Google Cloud | MongoDB 

1. Set the kubernetes context to your mongodb master cluster for example
...
## Deploy the MongoDB Ops Manager
 Reference Documentation :  Mastering MongoDB Ops Manager on Kubernetes | MongoDB 

Set the kubernetes context to your mongodb master cluster for example i.e same as the cluster above where the mongodb enterprise operator was deployed
...
1. Make sure that the operator is in the runningstate.
...
3. Deploy the Ops Manager using the opsmanagers Custom Resource, creating MongoDBOpsManager object, using the following manifest:
...
4. Verify Ops Manager and Ops Manager MongoDB application database pods running.
...
10. Using the username and password stored in the om-admin-secret (as created in step 5a ), we can log in to the Ops Manager User Interface using the address in the $URL variable.



11. Kubernetes Operator was in the Ops Manager ops-manager-db organization and the ops-manager-db project.

Open Screenshot from 2023-03-13 17-38-42-20230313-120842.png

12. If we click on the project ops-manager-db, we will be redirected to the panel where we can see the database pods of the Ops Manager application. Ops Manager monitors this database.

Open Screenshot from 2023-03-13 17-41-55-20230313-121155.png

Open Screenshot from 2023-03-13 17-47-18-20230313-121718.png


## Deploying MongoDB Across Multiple Kubernetes Clusters With MongoDBMulti
Reference Documentation: Deploying MongoDB Across Multiple Kubernetes Clusters With MongoDBMulti | MongoDB 

Pre-requisites

Set the environment variables as given below

We need to create the required variables: MASTER for a master Kubernetes cluster, and MDB_1, MDB_2, and MDB_3 for clusters which will host MongoDB replica set members. Important note: These variables should contain the full Kubernetes cluster names. for example if you have following clusters
...
In case this script fails due to the k8s cluster version is greater than 1.23 as service accounts don't automatically creates secrets 
...
6g). Check if the MongoDB Enterprise Operator multi cluster pod on the master cluster is running.
...
6h). It's now time to link all those clusters together using the MongoDB Multi CRD. The Kubernetes API has already been extended with a MongoDB-specific object - mongodbmulti.
...
6i). optional:  Review after the installation logs and ensure that there are no issues or errors.



POD=$(kubectl -n mongodb get po|grep operator|awk '{ print $1 }')
kubectl -n mongodb logs -f po/$POD -c mongodb-enterprise-operator
6j). We need to configure the required service accounts for each member cluster.
...
6k). optional : Verify the Expected sample output after executing the above command
...
6l). let's generate Ops Manager API keys and add our IP addresses to the Ops Manager access list. Get the Ops Manager  URL as explained in step 5g.
...
echo $URL
```
6m). Optional: Verify the Sample output
...
6n). Log in to Ops Manager(as covered in step 5i), and generate public and private API keys. When you create API keys, don't forget to add your current IP address to API Access List.

To do so, log in to the Ops Manager and go to ops-manager-db organization.


6o). Click Access Manager on the left-hand side, and choose Organization Access then choose Create API KEY  in the top right corner.

Open Screenshot from 2023-03-13 19-32-59-20230313-140259.png

6p). The key must have a name (example avesha) and permissions must be set to Organization Owner

Open Screenshot from 2023-03-13 19-34-28-20230313-140428.png

6q). When you click Next, you will see your Public Keyand Private Key. Copy those values and save them 
...
Open Screenshot from 2023-03-13 19-36-52-20230313-140652.png

6r). Get the public and private keys generated by the API key creator and paste them into the Kubernetes secret 
...
6s). You also need an  Organization ID. You can see the organization ID by clicking on the gear icon in the top left corner.


6t). Copy the Organization ID and paste to the Kubernetes config map below.
...
EOF
```
Step-7: create a MongoDB replica set using the MongoDBMultiCRD
...
7a). Apply the following command in the master cluster after setting the environment variables as discussed above
...
Open Screenshot from 2023-03-13 19-57-58-20230313-142758.png

7c). Verify our multi cluster ready is for use! We can verify this by displaying the 'mongodb multi' object.

In our case it is in reconciling state as replicaset is not able to communicate with each other 
...
## Create Service export for each member cluster as given below

Apply the following command in member cluster where multi-replica-set-0-0 is running
...
Apply the following command in member cluster where multi-replica-set-1-0 is running
...
Apply the following command in member cluster where multi-replica-set-2-0 is running
...
EOF
```
7e).After the service exports are applied in all three clusters then verify the service imports in all three clusters 
...
7f). Verify the host mapping for multi replicaset deployment is pointing to nsm IP’s from ops manager console 



7f). Now in master cluster again verify the multi cluster is ready is for use! We can verify this by displaying the 'mongodb multi' object as discussed in step 7c now as you can observe the state is running
...
7g). Verify the multi replicaset deployment from ops manager console