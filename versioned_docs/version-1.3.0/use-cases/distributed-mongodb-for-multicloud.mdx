...
- Use the cluster-1.config, cluster-2.config, cluster-3.config kubeconfig files for the k8s-cluster-1, k8s-cluster-2, and k8s-cluster-3 respectively.
...
- Create a slice called mongodb-slice across three worker clusters and onboard the mongodb namespace.
...
- Create the ServiceExport. The service export will be created in all 3 clusters as each cluster will have one Replicaset.
...
2. Verify the `merged.config` kubeconfig file:
...
3. Prepare the configuration topology file that you will use to install KubeSlice. Refer to the 
...
1. Set the context to the controller cluster.
...
2. Create the sample configuration YAML. he following is the sample slice configuration yaml file: 
...
2. Apply the slice configuration yaml file on the project namespace.
   ...
   ```
   kubectl apply -f demo-slice.yaml -n kubeslice-mongodb-project
   ```
