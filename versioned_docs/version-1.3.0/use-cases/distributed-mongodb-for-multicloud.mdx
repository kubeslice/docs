...
::note
Setting the environment variables is prerequisite.
:::
...
Follow these steps after you set the environment variables:
...
13. Click the project `ops-manager-db`. You are redirected to the panel that displays the database pods 
    of the Ops Manager application. The Ops Manager monitors this database.

![mongodb](/images/version1.3.0/use-cases/configure-mongodb/save-api-key-info.png)

14. Get the public and private keys generated by the API key creator and paste them into the Kubernetes secret. Apply the 
  below command in the master cluster 
