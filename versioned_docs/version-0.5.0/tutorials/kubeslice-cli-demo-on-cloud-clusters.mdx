...
To register a new worker cluster with the existing KubeSlice configuration (or KubeSlice Controller), follow these steps:
...
The following is an example custom topology file for registering a new worker cluster. Under worker, add a
