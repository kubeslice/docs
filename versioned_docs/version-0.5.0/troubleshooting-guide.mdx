...
3. If the connection issue still persists, check if the KubeSlice Controller endpoint and token in the cluster are correct in the
   ...
   see the [Slice Operator YAML file](/versioned_docs/version-0.5.0/getting-started-with-cloud-clusters/installing-kubeslice/registering-the-worker-cluster.mdx#create-the-slice-operator-yaml-file).
   file.
...
Cluster](/versioned_docs/version-0.5.0/getting-started-with-cloud-clusters/installing-kubeslice/registering-the-worker-cluster.mdx#get-the-secrets-of-the-registered-cluster).
