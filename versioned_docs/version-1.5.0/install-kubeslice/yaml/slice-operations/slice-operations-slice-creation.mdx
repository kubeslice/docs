...
:::note
The `sliceIPAMType` value `Local` is deprecated. Starting from KubeSlice 1.15.0, use `Static` for static IPAM or `Dynamic` for dynamic IPAM. 
Existing slices configured with Local IPAM will continue to be supported, but this type will be removed in future releases.
:::
...
| sliceIpamType                                                        | String                                         | It is the type of the IP address management for the slice subnet. The default values is Static. Set the value to Dynamic if you want to enable Dynamic IPAM.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Mandatory |
| subnetSize                                                           | Integer                                        | The subnet size to be allocated to each cluster when a cluster joins the slice. The default subnet size is /24. The range is `16` to `30`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Optional  |
...
| [vpnConfig](#openvpn-configuration)                                  | Object                                         | The Slice VPN Gateway is a slice network service component that provides a secure VPN tunnel between multiple clusters that are a part of the slice configuration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Optional  |
...
| Parameter                                                         | Parameter Type | Description                                                                                                                                       | Required  |
| ----------------------------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| sliceGatewayType                                                  | String         | The slice gateway type for inter cluster communication. Set the gateway type to `OpenVPN` or `Wiregaurd`.                                         | Mandatory |
...
| [SliceGatewayServiceType](#slice-gateway-service-type-parameters) | Object         | The type of gateway connectivity to use on a cluster. The default value is NodePort.                                                              | Optional  |

#### Slice Gateway Service Type Parameters

These parameters are related to the slice gateway service type parameters that provides an option to use the external Load Balancer. The user
can add the slice gateway type and protocol for each cluster.
...
This parameter is related to the open VPN certificate generation and is configured in the [slice configuration](#slice-creation) YAML file.
...
| Parameter                                                   | Parameter Type | Description                                                                                                                                                                                                                                                                     | Required  |
| ----------------------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| [applicationNamespaces](#application-namespaces-parameters) | Array object   | Defines the [namespaces that will be onboarded](#manage-namespaces) to the slice and their corresponding worker clusters.                                                                                                                                                       | Mandatory |
...
spec:
  ...
  sliceGatewayProvider:
    sliceGatewayType: OpenVPN  # Set this value to Wiregaurd if you want to use Wireguard as your VPN provider
    sliceCaType: Local
  sliceIpamType: Local #  The default value is 
...
spec:
  ...
  sliceGatewayProvider:
    sliceGatewayType: OpenVPN # # Set this value to Wiregaurd if you want to use Wireguard as your VPN provider
    sliceCaType: Local
  sliceIpamType: Local    The default values is Static. Set the value to Dynamic if you want to enable Dynamic IPAM
...
KubeSlice supports cluster connectivity among public and private clusters. Kubeslice extends the support to private clusters
using a Load Balancer. In the slice configuration YAML file, the user can specify the type of gateway connectivity to use
on a cluster. The type can be either NodePort or LoadBalancer. If you do not use LoadBalancer as the gateway connectivity
type, then NodePort is used by default. Like other configuration options in KubeSlice, the \* (asterisk) wildcard
character is allowed to indicate all clusters of a slice.

Add the gateway service type and protocol and the corresponding clusters under the sliceGatewayServiceType in the slice
configuration YAML as illustrated below.
...
After the LoadBalancer gateway service type and TCP/UDP gateway protocol are set, they become immutable.
However, if you want to change the gateway service type and protocol, then follow these steps:

1. Detach the cluster from the slice by removing it from the attached slice configuration , that is under `clusters` and remove
   the cluster, its protocol and type under `sliceGatewayServiceType` in the slice configuration YAML.
