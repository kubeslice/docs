...
| Dynamic IPAM             | On-Demand Subnet Allocation                  | Replaces static subnet pre-allocation with intelligent, on-demand allocation. Subnets are assigned only when clusters join a slice, not upfront during slice creation.
...
:::info 
KubeSlice uses Static IPAM by default. * KubeSlice uses Static IPAM by default. You must set the `sliceIPAMType` parameter to `Dynamic` in the slice 
