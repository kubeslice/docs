...
### Key Differences Between Static and Dynamic IPAM

- **Static IPAM**: Pre-allocates all 256 subnets at the time of slice creation, resulting in significant inefficiency—typically more than 99% of the allocated IP space remains unused.
- **Dynamic IPAM**: Allocates subnets only when clusters join the slice, achieving near 0% IP waste and ensuring optimal utilization of the IP pool.

Dynamic IPAM eliminates IP address waste while preserving full backward compatibility for existing Static IPAM–based slices.
