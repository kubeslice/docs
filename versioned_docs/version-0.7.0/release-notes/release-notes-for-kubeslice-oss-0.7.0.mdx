...
### Controller Events 
The controller events are enabled by default. The events are stored in the ``kubeslice-controller`` 
and ``kubeslice-project`` namespaces that can be retrieved using the corresponding YAML command.
