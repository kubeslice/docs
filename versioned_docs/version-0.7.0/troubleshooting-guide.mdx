...
This topic describes a few troubleshooting scenarios that you could face
...
3. If the connection issue still persists, check if the KubeSlice Controller endpoint and token in the cluster are correct in the
   ...
   see the [Slice Operator YAML file](/versioned_docs/version-0.7.0/getting-started-with-cloud-clusters/installing-kubeslice/registering-the-worker-cluster.mdx#create-the-slice-operator-yaml-file).
   file.
...
Cluster](/versioned_docs/version-0.7.0/getting-started-with-cloud-clusters/installing-kubeslice/registering-the-worker-cluster.mdx#get-the-secrets-of-the-registered-cluster).
...
### Why does the KubeSlice Manager dashboard's health tab show that a worker cluster's health status is unavailable?

![alt](/images/version8/km-kubeslicemanager/dash-health-worker-operator-down.png)

When a Worker Operator on a worker cluster is down or unable to communicate with the KubeSlice Controller, that
cluster's health status becomes unavailable. On the slice health pie chart, hovering your mouse indicates that the
component of only that worker cluster is down.

To resolve this issue, check the corresponding Worker Operator and restore its communication with the
KubeSlice Controller.

`kubeslice-operator` (Worker Operator) is a deployment running in the `kubeslice-system` namespace.
You can verify the status of `kubeslice-operator` on the corresponding worker cluster to further
trace the issue.

To verify the status of `kubeslice-operator`:

1. Get the details of `kubeslice-operator` using the following command:

   ```
   kubectl get deployment -n kubeslice-system --selector=control-plane=controller-manager
   ```
   ...
   kubeslice-operator  1/1    1           1          17d
   ```

2. Get the status of `kubeslice-operator` pods using the following command:

   ```
   kubectl get pods -n kubeslice-system --selector=control-plane=controller-manager
   ```
