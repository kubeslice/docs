...
After successfully installing KubeSlice Controller and the Slice Operator on the worker clusters. The `kubeslice-cli install --profile=minimal-demo` 
command creates a slice called `demo`. To validate the demo slice, see [Validating the Slice](#validating-the-slice).

On the `demo` slice, you can now onboard the iperf application. To onboard the application on the slice, see
[Deploying the iPerf Application](#deploying-the-iperf-application). 

You can also use the kubeslice-cli command to create a new slice where you can onboard application. To create a slice on your demo setup, 
follow these steps.
