...
This topic describes the steps to install KubeSlice on cloud clusters. To install KubeSlice Controller and its components on 
...
- You have cluster administrator privileges to install the KubeSlice Controller on controller cluster and Slice Operator on worker clusters.

## Installing KubeSlice

In this demonstration install KubeSlice on cloud clusters, using the custom topology configuration YAML.
...
[topology configuration](/versioned_docs/version-0.4.0/reference/sample-configuration-file.mdx#create-a-topology-configuration-file-for-cloud-clusters) file.
...
To uninstall KubeSlice from your cloud clusters, see [Uninstalling KubeSlice](/versioned_docs/version-0.4.0/kubeslice-cli/uninstalling-kubeslice.mdx).