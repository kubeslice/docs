...
:::caution
The `kubeslice-cli create sliceConfig -n <project-namespace> -f <slice-configuration-yaml>` command returns success after the slice 
configuration is applied. However, in each cluster, the relevant pods for controlling and managing the slice may still be starting. Ensure to wait for 
the slice to complete the initialization before deploying services to it.
:::
...
To uninstall KubeSlice from your cloud clusters, follow the instructions in [Uninstall KubeSlice](/versioned_docs/version-0.4.0/kubeslice-cli/uninstalling-kubeslice.mdx).