...
Example
```
sudo curl -fL https://github.com/kubeslice/kubeslice-cli/releases/download/0.5.3/kubeslice-cli-0.5.3-linux-amd64  -o /usr/local/bin/kubeslice-cli
```
...
[Kubeslice Configuration Parameters](/versioned_docs/version-1.0.0/reference/configuration-parameters.mdx). 
...
see [Uninstalling KubeSlice](/versioned_docs/version-1.0.0/kubeslice-cli/uninstalling-kubeslice.mdx).
...
1. To uninstall the KubeSlice components on kind clusters and delete the kind clusters created using `full-demo|minimal-demo|enterprise-demo` option, 
